// Created by Verilog::EditFiles from or1200.v
...
module or1200_rf(
	...
	// Write i/f
	wb_freeze, rf_addrw, rf_dataw,id_freeze, flushpipe,

	// Read i/f
	rf_addra, rf_rda, rf_dataa,  rf_addrb,rf_rdb, rf_datab,
	// Debug
	spr_we, spr_addr, spr_dat_cpu, spr_dat_rf,
...
output const_zero_rf;
output rf_we;
output [31:0] const_zero_data_rf;
output [`OR1200_OPERAND_WIDTH-1:0]		rf_dataw_out;
input [`OR1200_OPERAND_WIDTH-1:0]		from_rfa;
input [`OR1200_OPERAND_WIDTH-1:0]		from_rfb;
...
wire				rfwb_op_0;
assign rfwb_op_0=rfwb_op[0];

input	[31:0]			spr_cs;		// Unit select
wire				spr_cs_OR1200_SPR_GROUP_SYS;
assign spr_cs_OR1200_SPR_GROUP_SYS= spr_cs[0];

input	[`OR1200_SR_WIDTH-1:0]	sr;
wire	sr_OR1200_SR_SM;
assign sr_OR1200_SR_SM = sr[`OR1200_SR_SM];
...
input	[`OR1200_OPERAND_WIDTH-1:0]		rf_dataw;
...
output	[`OR1200_OPERAND_WIDTH-1:0]		rf_dataa;
output	[`OR1200_OPERAND_WIDTH-1:0]		rf_datab;
input				rf_rda;
input				rf_rdb;
...
wire	[`OR1200_OPERAND_WIDTH-1:0]		rf_dataw_out;
wire				rf_we;
wire				spr_valid;
wire				rf_ena;
wire				rf_enb;
...
//
// SPR access is valid when spr_cs_OR1200_SPR_GROUP_SYS is asserted and
// SPR address matches GPR addresses
//
assign spr_valid = spr_cs_OR1200_SPR_GROUP_SYS & (spr_addr[10:5] == `OR1200_SPR_RF);

//
// SPR data output is always from RF A
//
assign spr_dat_rf = from_rfa;

//
// Operand A comes from RF or from saved A register
//
assign rf_dataa = (dataa_saved[32]) ? dataa_saved[31:0] : from_rfa;

//
// Operand B comes from RF or from saved B register
//
assign rf_datab = (datab_saved[32]) ? datab_saved[31:0] : from_rfb;

//
// RF A read address is either from SPRS or normal from CPU control
//
assign rf_addra = (spr_valid & !spr_we) ? spr_addr[4:0] : rf_addra;

//
// RF write address is either from SPRS or normal from CPU control
//
assign rf_addrw = (spr_valid & spr_we) ? spr_addr[4:0] : rf_addrw;

//
// RF write data is either from SPRS or normal from CPU datapath
//
assign rf_dataw_out = (spr_valid & spr_we) ? spr_dat_cpu : rf_dataw;
...
assign rf_we = ((spr_valid & spr_we) | (rfwb_op_0 & ~wb_freeze)) & rf_we_allow & (sr_OR1200_SR_SM | (|rf_addrw));

//
// CS RF A asserted when instruction reads operand A and ID stage
// is not stalled
//
assign rf_ena = rf_rda & ~id_freeze | spr_valid;	// probably works with fixed binutils
...
// is not stalled
//
assign rf_enb = rf_rdb & ~id_freeze | spr_valid;
// assign rf_enb = 1'b1;
...
assign const_zero_rf = 1'b0;
wire [31:0] const_zero_data_rf;
assign const_zero_data_rf = 32'b00000000000000000000000000000000;
