...
static void processComplexBlock(INOUTP ezxml_t Parent, INOUTP t_block *cb, INP int index, 
	...
	//fbmao debug
//
//	if(my_atoi(tokens[2].data) != index)
//	{
//	  printf("data %d,  index %d\n", my_atoi(tokens[2].data),index);
//	}


//fbmao debug  assert(my_atoi(tokens[2].data) == index);  //should uncomment in order to avoid error
	found = FALSE;
	...
	while(Cur) {
		if(0 == strcmp(Cur->name, "port"))
		{
			...
			if(0 == strcmp(Parent->name, "inputs")) {
				...
				//fbmao debug2
				printf("numtokens %d, num_input_pins[in_port  %d] %d\n", num_tokens,in_port, pb->pb_graph_node->num_input_pins[in_port]);
				...
				if(num_tokens != pb->pb_graph_node->num_input_pins[in_port]) {
					...
					//fbmao debug3 virtaul net
			//	    if(0 == virtualpinflag) //consider virtualpin
					     exit(1);
				}
			...
			if(0 == strcmp(Parent->name, "inputs") || 0 == strcmp(Parent->name, "globals")) {
				if(pb->parent_pb == NULL) {
					...
					//fbmao debug2 virtual pins
					//num_tokens = pb->pb_graph_node->num_input_pins[in_port];
					...
					for(i = 0; i < num_tokens; i++) {
                       ...
                       //fbmao debug
					//	printf("rr_node_index %d, netnum %d\n", rr_node_index, add_net_to_hash(nhash, pins[i], ncount));
						//fbmao debug3
//						if(rr_node_index == 31)
//						{
//							printf("Error: bug comes out, pls check.\n");
//
//							continue;
//						}
//		     			if(i > 35)
//							continue;
//						if(rr_graph == NULL || pins[i] == NULL)
//							continue;
						...
						rr_graph[rr_node_index].net_num = add_net_to_hash(nhash, pins[i], ncount);
	                       //fbmao debug
							printf("rr_node_index %d, netnum %d\n", rr_node_index, rr_graph[rr_node_index].net_num);
					}
				} else {
			...
			if(0 == strcmp(Parent->name, "outputs")) {
				if(pb->pb_graph_node->pb_type->num_modes == 0) {
					...
					for(i = 0; i < num_tokens; i++) {
						...
						//fbmao debug3 virtaul net
//					    if(1 == virtualpinflag) //consider virtualpin
//					    	if(i > 32) continue;
						printf("rr_node_index %d, netnum %d\n", rr_node_index, add_net_to_hash(nhash, pins[i], ncount));
...
//					//fbmao debug3
					if(count[netnum] > (*ext_nets)[netnum].num_sinks)
					{
						...
						printf("count[netnum]  %d  (*ext_nets)[netnum].num_sinks %d", count[netnum], (*ext_nets)[netnum].num_sinks);
						printf("   netnum %d, netname %s\n", netnum, (*ext_nets)[netnum].name);
						fflush(stdout);
			//			exit(1);
					}
