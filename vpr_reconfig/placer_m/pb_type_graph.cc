...
void alloc_and_load_all_pb_graphs () {
	...
	for(i = 0; i < num_types; i++) {
		if(type_descriptors[i].pb_type) {
	////		load_pb_graph_pin_to_pin_annotations(type_descriptors[i].pb_graph_head);   ///may have bug
		}
	}
}
...
static void alloc_and_load_pb_graph(INOUTP t_pb_graph_node *pb_graph_node, 
	...
	pb_graph_node->child_pb_graph_nodes = (s_pb_graph_node ***)my_calloc(pb_type->num_modes, sizeof(t_pb_graph_node **));
	for(i = 0; i < pb_type->num_modes; i++) {
		pb_graph_node->child_pb_graph_nodes[i] = (s_pb_graph_node **)my_calloc(pb_type->modes[i].num_pb_type_children, sizeof(t_pb_graph_node *));
		for(j = 0; j < pb_type->modes[i].num_pb_type_children; j++) {
			pb_graph_node->child_pb_graph_nodes[i][j] = (s_pb_graph_node *)my_calloc(pb_type->modes[i].pb_type_children[j].num_pb, sizeof(t_pb_graph_node));
 ...
 */
t_pb_graph_pin *** alloc_and_load_port_pin_ptrs_from_string(INP  t_pb_graph_node *pb_graph_parent_node,   //fbmao remove const
																  INP t_pb_graph_node **pb_graph_children_nodes,
																  INP  char * port_string,   //fbmao remove const 
																  OUTP int ** num_ptrs,
																  OUTP int * num_sets,
																  INP boolean is_input_to_interc,
																  INP boolean interconnect_error_check) {
...
static void alloc_and_load_complete_interc_edges(INP t_interconnect *interconnect,
	...
	cur = (s_linked_vptr *)my_malloc(sizeof(struct s_linked_vptr));
	cur->next = edges_head;
	edges_head = cur;
	...
	for(i_inset = 0; i_inset < num_input_sets; i_inset++) {
		for(i_inpin = 0; i_inpin < num_input_ptrs[i_inset]; i_inpin++) {
			input_pb_graph_node_pin_ptrs[i_inset][i_inpin]->output_edges = (s_pb_graph_edge **)my_realloc(input_pb_graph_node_pin_ptrs[i_inset][i_inpin]->output_edges,
	...
	for(i_outset = 0; i_outset < num_output_sets; i_outset++) {
		for(i_outpin = 0; i_outpin < num_output_ptrs[i_outset]; i_outpin++) {
			output_pb_graph_node_pin_ptrs[i_outset][i_outpin]->input_edges = (s_pb_graph_edge **)my_realloc(output_pb_graph_node_pin_ptrs[i_outset][i_outpin]->input_edges,
...
static void alloc_and_load_direct_interc_edges(  INP t_interconnect *interconnect,
	...
	for(i = 0; i < num_input_ptrs[0]; i++) {		
		input_pb_graph_node_pin_ptrs[0][i]->output_edges = (s_pb_graph_edge **)my_realloc(
		...
		output_pb_graph_node_pin_ptrs[0][i]->input_edges = (s_pb_graph_edge **)my_realloc(
...
static void alloc_and_load_mux_interc_edges(	INP t_interconnect * interconnect,
	...
	for(i_outpin = 0; i_outpin < num_output_ptrs[0]; i_outpin++) {
		output_pb_graph_node_pin_ptrs[0][i_outpin]->input_edges = (s_pb_graph_edge **)my_realloc(output_pb_graph_node_pin_ptrs[0][i_outpin]->input_edges,
	...
	for(i_inset = 0; i_inset < num_input_sets; i_inset++) {
		...
		edges[i_inset].input_pins = (t_pb_graph_pin **)my_calloc(num_output_ptrs[0], sizeof(t_pb_graph_pin *));
		edges[i_inset].output_pins = (t_pb_graph_pin **)my_calloc(num_output_ptrs[0], sizeof(t_pb_graph_pin *));
...
////static
/////fbmao revised  remove static
void print_tabs(FILE *fpout, int num_tabs) {
	int i;
