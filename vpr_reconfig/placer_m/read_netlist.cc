...
//////////////------begin  fbmao add for global variables
			 ...
			 OUTP struct s_net *net_list[])
{
	...
	blist = (s_block*)my_calloc(bcount, sizeof(t_block));
	nhash = alloc_hash_table();
	ncount = 0;

	logical_block = (s_logical_block*)my_calloc(num_saved_logical_blocks, sizeof(t_logical_block));
...
static void processComplexBlock(INOUTP ezxml_t Parent, INOUTP t_block *cb, INP int index, 
	...
	rr_node = (t_rr_node*)my_calloc(cb[index].type->pb_graph_head->total_pb_pins, sizeof(t_rr_node));
	alloc_and_load_rr_graph_for_pb_graph_node(cb[index].pb->pb_graph_node, arch, 0);      ////rr_node no use   /// fbmao  may have bug
...
static void processPb(INOUTP ezxml_t Parent, INOUTP t_pb* pb, INOUTP t_rr_node *rr_graph,
	...
	if(pb_type->num_modes == 0) {
		...
		pb->child_pbs = (s_pb**)my_calloc(pb_type->modes[pb->mode].num_pb_type_children, sizeof(t_pb*));
		for(i = 0; i < pb_type->modes[pb->mode].num_pb_type_children; i++) {
			pb->child_pbs[i] = (s_pb*)my_calloc(pb_type->modes[pb->mode].pb_type_children[i].num_pb, sizeof(t_pb));
		}
...
static struct s_net *alloc_and_init_netlist_from_hash(INP int ncount, INOUTP struct s_hash **nhash) {
	...
	nlist = (s_net*)my_calloc(ncount, sizeof(struct s_net));
	...
	while(curr_net != NULL) {
		...
		nlist[curr_net->index].node_block = (int*)my_malloc(curr_net->count * sizeof(int));
		nlist[curr_net->index].node_block_pin = (int*)my_malloc(curr_net->count * sizeof(int));
...
static void alloc_internal_cb_nets(INOUTP t_pb *top_level, INP t_pb_graph_node *pb_graph_node, INOUTP t_rr_node *rr_graph, INP int pass) {
			...
			for(j = 0; j < pb_graph_node->num_input_pins[i]; j++) {
				if(rr_graph[pb_graph_node->input_pins[i][j].pin_count_in_cluster].net_num != OPEN) {
					if(pass == 2) {
						...
						top_level->local_nets[top_level->num_local_nets].node_block = (int*)my_calloc(num_sinks, sizeof(int));
						top_level->local_nets[top_level->num_local_nets].node_block_port = (int*)my_calloc(num_sinks, sizeof(int));
						top_level->local_nets[top_level->num_local_nets].node_block_pin = (int*)my_calloc(num_sinks, sizeof(int));
					}
	...
	if(pb_graph_node->parent_pb_graph_node == NULL) { /* at top level */
		if(pass == 1) {
			top_level->local_nets = (s_net*)my_calloc(top_level->num_local_nets, sizeof(struct s_net));
		}
	}
}
...
static void mark_constant_generators_rec(INP t_pb *pb,
	...
	if(pb->pb_graph_node->pb_type->blif_model == NULL) {
		...
		if(const_gen == TRUE) {
	///fbmao comment		printf("%s is a constant generator \n", pb->name);
