...
{
	///fbmao debug 
///	cout << "In function  SA_Floorplan xxxxx debug xxxxxxxx" << endl;
//	double debugtime = seconds();
//	return debugtime;
...
////	int N = k * fp.size();
	...
	//////cout << "Debug K value In function  SA_Floorplan xxxxx debug xxxxxxxx" << endl;
	//////k = 1;

	//fbmao revised
	int N = k * fp.modNumBeforeSplit();
	...
	float T, actual_T = 1;
	double avg = init_avg;
	float conv_rate = 0.96; //// original 0.99; fbmao change
	double time;
	double time_start;
	...
	///fbmao debug
	//////////avg = 0.009; // Random_Floorplan(fp, N);       ////first  avg   do not consider split.  later can consider split
	//////////cout << "xxxxxxxxxxxxxxFor debug, comment the Random_Floorplan function, in SA_Floorplan function";
	...
	//fbmao new add
	////if do split operation. Before SA algorithm, it must need to return back to original (top) modules.
	...
	///for support split after some time
	...
	do{
		...
		*/
		...
		if (issplitaftersometime)   ////split in one ratio.     (for modules with multi-ratio flow.)
		{   ///only excute one time
			if (count >=2)    ///before result. intersol count is 1.   can change .  based on the termination condition. choose a intersol as the initial sol
			{
				issplitInCurrentratio = 1;  //control the place_module() function. only choose one ratio

				////store current placement result
				fp.recover_best();
				fp.packing();     //isoneratioflow.  keep the ratio

				//fbmao revised
				fp.checkIOPositionEnoughAndExpandwidthAndHeight();
				fp.doLocalplaceForEachmoduleMapToArch();
				fp.getFinalPlaceFiles(const_cast<char*>((fp.getFilenamefromFP()).c_str()));   //store results. it has relation with issplitaftersometime
				
				//Then it will do split operation.   change the moverate swap rate
				////isinternosplitoneratio  1: do not split    0: split for inter solution
				if (!isinternosplitoneratio)
				{
					swap_rate = tmpswaprate;  ///now it can do split ratio	
				}

                ///output
				printf("Interarea %.2f\n", fp.getArea());
				printf("splitsolution time  = %.2f\n", time);
				printf("running time  = %.2f\n", seconds());


				issplitaftersometime = 0; //split after this step. And this loop will not run

				// re-get new inital solution  (based on split)
				pre_cost = best = fp.getCost();   ////when issplitaftersometime == 0. it has different styles for getting cost

				///Notice:   reset parameters (rerun SA)  ----based on the inter solution
				T = actual_T; ////return back T value.  and increase the P value to avoid be in the local minimum
				count = 0;


				///normalize wirelength, area
		////	fp.normalize_cost(200);   //avoid be in the local minimum . can chage the P acceprate. but do not impact the flow.

				////if (!isinternosplitoneratio)   ///??? check this code
				////{
				////	T = actual_T * 0.9; ////return back T value.  and increase the P value to avoid be in the local minimum
				////}
...
/////		cout << "test bug. In SA function " << endl;

	} while (((reject_rate<conv_rate && T>1e-3) || bad_count <= 3) && (!io3 || count < 10));/// 10)); ///2)); 
/////	} while (((reject_rate<conv_rate && T>1e-3) || bad_count <= 3) && (!io3 || count < 1)); /////dubug will delete later


	///while (((reject_rate<conv_rate && T>1e-3) || bad_count <= 3));// && (!io3 || count < 10)); ///2));  // for test 
